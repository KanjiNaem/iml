...
def transform_data(X):
    ...
    for i in range(700): 
        for j in range(5):
            linear[j] = X[i][j]
            quadratic[j] = X[i][j] ** 2
            exponential[j] = np.exp(X[i][j])
            cosine[j] = np.cos(X[i][j])
        X_input[i] = np.concatenate((linear, quadratic, exponential, cosine, constant))        
...
def fit(X, y):
    ...
    model = Ridge(alpha=1.0).fit(X_input,y)
    weights = model.coef_   
